...
export default function EventBasedGatewayBehavior(eventBus, modeling) {
  ...
  this.preExecuted('shape.replace', function(event) {
    ...
    var newShape = event.context.newShape,
        newShapeOutgoingConnections = newShape.outgoing;
    ...
    newShapeOutgoingConnections
      .filter(isSequenceFlow)
      .forEach(function(newShapeSequenceFlow) {
        newShapeSequenceFlow.target.incoming
          .filter(isSequenceFlow)
          .forEach(function(newShapeTargetIncomingSequenceFlow) {
            if (newShapeTargetIncomingSequenceFlow.source !== newShape) {
              modeling.removeConnection(newShapeTargetIncomingSequenceFlow);
            }
          });
      });
  });
